...
!!! tip "Сначала запустите `doctor`"
    `./emergency-callback doctor` проверяет окружение, БД, миграции, AMI/ARI,
    Eskiz, sox, аудиофайлы и веб-сервис — и часто сразу указывает на причину.
...
| Звонок отвечает, подсказка проигрывается, но нажатия клавиш не регистрируются (односторонний звук) | NAT-настройки АТС отправляют RTP на внешний адрес, потому что сеть приложения/абонента не указана в Local Networks. Решение: Settings → Asterisk SIP Settings → Local Networks (добавьте подсеть) либо исправьте `external_media_address`. |
